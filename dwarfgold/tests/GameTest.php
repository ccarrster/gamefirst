...
class JimmyTest extends TestCase
{
    ...
    public function testElfArrowWin(){
        ...
        foreach($teams as $team){
            ...
            if($team->hasFaction("Mage")){
                $gold = $team->getGoldPiles();
                $this->assertEquals(2, count($gold));
            }
        }
    }
    public function testElfArrowWinTwo(){
        $game = new Game();
        $player = new Player();
        $key = $game->addPlayer($player);
        $game->chooseFaction($key, "Mage");
    ...
    public function testElfArrowWinTwoOnOne(){
        $game = new Game();
        $player = new Player();
        $key = $game->addPlayer($player);
        $game->chooseFaction($key, "Mage");
